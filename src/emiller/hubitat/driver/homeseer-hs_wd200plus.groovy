...
import hubitat.zwave.commands.basicv1.BasicReport
import hubitat.zwave.commands.basicv1.BasicSet
import hubitat.zwave.commands.switchmultilevelv1.SwitchMultilevelReport
import hubitat.zwave.commands.switchmultilevelv1.SwitchMultilevelSet
import hubitat.zwave.commands.configurationv1.ConfigurationReport
import hubitat.zwave.commands.hailv1.Hail
import hubitat.zwave.commands.manufacturerspecificv2.ManufacturerSpecificReport
import hubitat.zwave.commands.versionv1.VersionReport
import hubitat.zwave.commands.firmwareupdatemdv2.FirmwareMdReport
import hubitat.zwave.commands.switchmultilevelv1.SwitchMultilevelStopLevelChange
import hubitat.zwave.commands.centralscenev1.CentralSceneNotification
...
List zwaveEvent(BasicReport cmd) {
    dimmerEvents(cmd)
}

List zwaveEvent(BasicSet cmd) {
    dimmerEvents(cmd)
}

List zwaveEvent(SwitchMultilevelReport cmd) {
    dimmerEvents(cmd)
}

List zwaveEvent(SwitchMultilevelSet cmd) {
    dimmerEvents(cmd)
}

private List dimmerEvents(Command cmd) {
    logDebug "dimmerEvents(Command cmd)"
    logTrace "cmd: $cmd"
...
def zwaveEvent(ConfigurationReport cmd) {
    logDebug "zwaveEvent(ConfigurationReport cmd)"
    logTrace "cmd: $cmd"
...
def zwaveEvent(Hail cmd) {
    logDebug "zwaveEvent(Hail cmd)"
    logTrace "cmd: $cmd"
    logInfo "Switch button was pressed"
    createEvent([name: "hail", value: "hail", descriptionText: "Switch button was pressed", displayed: false])
}

def zwaveEvent(ManufacturerSpecificReport cmd) {
    logDebug "zwaveEvent(ManufacturerSpecificReport cmd)"
    logTrace "cmd: $cmd"
...
def zwaveEvent(VersionReport cmd) {
    logDebug "zwaveEvent(VersionReport cmd)"
    logTrace "cmd: $cmd"
...
def zwaveEvent(FirmwareMdReport cmd) {
    logDebug "zwaveEvent(FirmwareMdReport cmd)"
    logTrace "cmd: $cmd"
...
def zwaveEvent(SwitchMultilevelStopLevelChange cmd) {
    logDebug "zwaveEvent(SwitchMultilevelStopLevelChange cmd)"
    logTrace "cmd: $cmd"
    logInfo "Stop level change on device ${device.label}"
    [createEvent(name: "switch", value: "on"), response(zwave.switchMultilevelV1.switchMultilevelGet().format())]
}

def zwaveEvent(Command cmd) {
    // Handles all Z-Wave commands we aren't interested in
    logDebug "zwaveEvent(Command cmd)"
    logTrace "cmd: $cmd"
    [: ]
}
...
def zwaveEvent(CentralSceneNotification cmd) {
    logDebug "zwaveEvent(CentralSceneNotification cmd)"
    logTrace "cmd: $cmd"
